...
pub fn main() {
    ...
    match fs::copy(&config_file, &dest_file) {
        Ok(_) => println!("BUILD: Copied config file successfully from {:?} to {:?}", config_file, dest_file),
        Err(e) => println!("BUILD: Failed to copy script from {:?} to {:?}: {:?}.", config_file, dest_file, e)
    }
    ...
    println!("BUILD: Attempting to move {:?} to {:?}", log_config_file, log_dest_file);

    match fs::copy(&log_config_file, &log_dest_file) {
        Ok(_) => println!("BUILD: Copied log config file successfully from {:?} to {:?}", log_config_file, log_dest_file),
        Err(e) => println!("BUILD: Failed to copy log config file from {:?} to {:?}: {:?}", log_config_file, log_dest_file, e)
    }
}